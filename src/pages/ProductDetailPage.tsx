    ...
    <>
      <div className="container mx-auto px-4 py-6 md:py-8 pb-28 md:pb-8">
        ...
        <nav className="flex items-center text-xs sm:text-sm text-muted-foreground mb-5 gap-1 overflow-x-auto">
          ...
          <Link to={`/products/${product.categorySlug}`} className="hover:text-foreground transition-colors whitespace-nowrap">{product.category}</Link>
          <ChevronRight className="h-3 w-3 flex-shrink-0" />
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-10">
          ...
          <div className="space-y-3">
            ...
            <div
              ref={imgRef}
              className="relative aspect-square bg-secondary rounded-md overflow-hidden group cursor-crosshair"
            ...
            >
              ...
              {/* Zoom hint (visible when not zooming) */}
              {!isZooming && (
          ...
          <div>
            ...
            <h1 className="font-display text-xl sm:text-2xl md:text-3xl font-bold text-foreground leading-tight mb-2">{product.name}</h1>
            <p className="text-xs text-muted-foreground mb-4">{t("products.sku")}: <span className="font-mono">{product.sku}</span></p>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="flex items-center gap-2.5 p-3 bg-secondary/60 rounded-md">
                ...
                <span className="text-xs text-muted-foreground leading-tight">{t("products.free_shipping_note")}</span>
              </div>
              <div className="flex items-center gap-2.5 p-3 bg-secondary/60 rounded-md">
            ...
            <div className="border border-border rounded-md overflow-hidden">
              <h3 className="font-display font-bold text-xs uppercase tracking-wider px-4 py-3 bg-secondary">{t("products.specifications")}</h3>
            ...
            {product.compatibility && product.compatibility.length > 0 && (
              <div className="border border-border rounded-md overflow-hidden mt-4">
                <h3 className="font-display font-bold text-xs uppercase tracking-wider px-4 py-3 bg-secondary">{t("products.compatibility")}</h3>
                <div className="px-4 py-3 flex flex-wrap gap-2">
                  {product.compatibility.map((c) => (
                    <span key={c} className="text-xs bg-secondary text-foreground px-2.5 py-1.5 rounded">{c}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
              {relatedProducts.map((rp) => (
                <div key={rp.id} className="product-card group flex flex-col">
                  <Link to={`/product/${rp.slug}`} className="block aspect-square overflow-hidden bg-secondary">
                  ...
                  <div className="p-3 md:p-4 flex flex-col flex-1">
                    ...
                    <Link to={`/product/${rp.slug}`} className="text-sm font-medium text-foreground hover:text-accent transition-colors line-clamp-2 mb-auto">
                      {rp.name}
                    </Link>
                    <p className="text-sm font-bold text-foreground mt-2">{formatPrice(rp.price)}</p>
                    <button onClick={() => addItem(rp)} className="mt-2 w-full btn-accent text-xs py-2 rounded-sm font-semibold uppercase tracking-wide">
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-card border-t border-border px-4 py-3 shadow-[0_-4px_12px_rgba(0,0,0,0.08)]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center border border-border rounded-sm flex-shrink-0">
            <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-2.5 py-2 hover:bg-secondary transition-colors" aria-label="Decrease">
              <Minus className="h-3.5 w-3.5" />
            </button>
            <span className="px-3 py-2 text-sm font-medium text-center border-x border-border min-w-[36px]">{qty}</span>
            <button onClick={() => setQty(qty + 1)} className="px-2.5 py-2 hover:bg-secondary transition-colors" aria-label="Increase">
      ...
      {/* ── Lightbox ── */}
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-foreground/95 flex items-center justify-center p-4 animate-fade-in" onClick={() => setLightbox(false)}>
          <button className="absolute top-4 right-4 text-background/80 hover:text-background transition-colors z-10" onClick={() => setLightbox(false)} aria-label="Close">
            <X className="h-7 w-7" />
          </button>
